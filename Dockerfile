...
COPY output/zoo-project-1.7.0/zoo-project/zoo-kernel/zoo_loader.cgi /var/www/zoo-bin/zoo_loader.cgi
COPY output/zoo-project-1.7.0/zoo-project/zoo-kernel/libzoo_service.so /usr/local/lib/libzoo_service.so 
COPY assets/zoo/httpd/htaccess /var/www/html/zoo/.htaccess
COPY assets/zoo/httpd/zoo.conf	/etc/httpd/conf.d/zoo.conf
COPY output/demo/updateStatus.xsl /var/www/data/updateStatus.xsl
COPY assets/main.cfg /etc/zoo-project/main.cfg
COPY output/demo/GetStatus.zcfg /opt/t2service/GetStatus.zcfg 
COPY output/demo/wps_status.zo /opt/t2service/wps_status.zo
